(() => {
    ...
    function initSliders() {
        if (document.querySelector(".filter__slider")) new swiper_core_Swiper(".filter__slider", {
            modules: [ Navigation ],
            observer: true,
            observeParents: true,
            slidesPerView: 4,
        ...
        if (document.querySelector(".newsletter__slider")) new swiper_core_Swiper(".newsletter__slider", {
            modules: [ Navigation ],
            observer: true,
            observeParents: true,
            slidesPerView: 6,
    ...
    function windowLoaded() {
        let keypressActions = e => {
            if (e.key === "Escape") document.documentElement.classList.remove("catalog-open");
        };
        let documentActions = e => {
            const targetElement = e.target;
            e.type;
            targetElement.tagName;
            if (targetElement.closest(".search-header__open")) document.documentElement.classList.toggle("search-open"); else if (!targetElement.closest(".search-header__body")) document.documentElement.classList.remove("search-open");
        };
        document.querySelectorAll("[data-filter-group]").forEach((block => {
            ...
            block.querySelectorAll("[data-filter]").forEach((button => {
                button.addEventListener("click", (() => {
                    ...
                    if (category === "all") selectedCategories = [ "all" ]; else {
                        let index = selectedCategories.indexOf(category);
                        if (index > -1) selectedCategories.splice(index, 1); else selectedCategories.push(category);
                        if (selectedCategories.length > 0) {
                            let allIndex = selectedCategories.indexOf("all");
                            if (allIndex > -1) selectedCategories.splice(allIndex, 1);
                        }
                    }
                    setActiveButtons(block, selectedCategories);
                    filterProducts(block, selectedCategories);
            ...
            function filterProducts(block, categories) {
                ...
                products.forEach((product => {
                    if (categories.includes("all") || categories.some((category => product.dataset.category.includes(category)))) product.classList.add("show"); else product.classList.remove("show");
                }));
            }
            function setActiveButtons(block, categories) {
                let buttons = block.querySelectorAll("[data-filter]");
                buttons.forEach((button => {
                    if (categories.includes(button.dataset.filter)) button.classList.add("active"); else button.classList.remove("active");
                }));
            }
            filterProducts(block, selectedCategories);
        }));
